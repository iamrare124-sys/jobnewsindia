...
const getSiteName = () => process.env.SITE_NAME || 'jobnewsindia'
...
export async function getPostBySlug(slug) {
  try {
    ...
    getSupabaseAdmin().from('posts').update({ views: (data.views || 0) + 1 }).eq('slug', slug).eq('site_name', siteName).then(() => {})
    return data
  } catch (err) { console.error('getPostBySlug error:', err.message); return null }
}

export async function getRelatedPosts(category, currentSlug, limit = 4) {
  try {
    const siteName = getSiteName()
...
async function enforcePostLimit() {
  try {
    ...
    if (oldest?.length) {
      ...
      console.log(`Enforced post limit: deleted ${oldest.length} old posts for site ${siteName}`)
    }
...
export async function searchPosts(query, limit = 20) {
  try {
    ...
    const { data, error } = await getSupabase()
      .from('posts')
      .select('id, title, slug, excerpt, category, tags, cover_image, published_at, created_at')
      .eq('site_name', siteName)
      .eq('published', true)
    ...
    if (error) throw error
    return data || []
  } catch (err) { console.error('searchPosts error:', err.message); return [] }
}

export async function getTotalCount(category = null) {
  try {
    const siteName = getSiteName()
    let query = getSupabase()
      .from('posts')
